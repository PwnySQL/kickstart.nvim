...
do
  ...
  -- Sync clipboard between OS and Neovim, e.g. using xsel or xclip.
...
do
  ...
  -- disable git information to save space
  ---@diagnostic disable-next-line: duplicate-set-field
  statusline.section_git = function() return '' end
  -- show only relative file name
  ---@diagnostic disable-next-line: duplicate-set-field
...
do
  ...
  -- Shortcut for grep with file globbing (thanks gemini!)
  vim.keymap.set('n', '<leader>sgg', function(opts)
    ...
    local finder = finders.new_async_job {
      command_generator = function(prompt)
        ...
        -- Split prompt by double space: "query  path_filter"
        -- Use plain = true to split based on chars and not by regexp (to be able to
        -- split on regexp special chars).
        local pieces = vim.split(prompt, '  ', { plain = true })
        local args = { 'rg' }

        -- Add default telescope vimgrep arguments
        for _, value in ipairs(conf.vimgrep_arguments) do
          if value ~= 'rg' then table.insert(args, value) end
        end

        -- Guard: Ensure we have a search term
        if not pieces[1] or pieces[1] == '' then return nil end

        -- First part is the search pattern
        table.insert(args, '-e')
        table.insert(args, pieces[1])

        -- Second part (after double space) is the glob pattern/path
        if #pieces >= 2 and not (#pieces == 2 and pieces[2] == '') then
    ...
    pickers
      .new(opts, {
        debounce = 100,
        prompt_title = 'Live Grep (Query  Path)',
        finder = finder,
        previewer = conf.grep_previewer(opts),
        -- Implement custom sorter to only highlight the grepped-for term.
        sorter = sorters.new {
          scoring_function = function()
            return 0 -- 0 means "dont filter"; we trust rg's results
          end,
          highlighter = function(_, prompt, display)
            local pieces = vim.split(prompt, '  ', { plain = true })
            local search_term = pieces[1] or ''
            -- This uses the standard highlighter logic but ONLY for the grepped-for term.
            return sorters.highlighter_only(opts):highlighter(search_term, display)
          end,
        },
      })
      :find()
  end, { desc = '[S]earch by [G]rep with [G]lob (separated by two spaces)' })
end
